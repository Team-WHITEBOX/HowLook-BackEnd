...
public interface TournamentDateRepository extends JpaRepository<TournamentDateInfo,Long>{
    ...
    @Query("select ti.tournamentDate from TournamentDateInfo ti where ti.configdateid = 1")
    Long selectTournamentDatefromTournamentDateInfo();

    //update문 @Query로 작성시 @Modifying @Transactional 붙여줘야 작동가능
    @Modifying
    @Transactional
    @Query("update TournamentDateInfo ti set ti.tournamentDate = ti.tournamentDate + 1 where ti.configdateid = 1")
    void updateTournamentDateToNextDay();

    @Modifying
    @Transactional
    @Query("update TournamentDateInfo ti set ti.tournamentDate = 0 where ti.configdateid = 1")
